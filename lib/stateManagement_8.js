...
class Observe_Charts_state {
	...
	set_filter(params_chart, params_charts_target, interaction_type) {
		...
		for (var y = 0; y < params_charts_target.length; y++) {
			...
			if (target_chart.chart_type !== "leaflet") {
				var activeLegends={};
				params_charts_target.map(c=> {
					if (c.chart_type !== "leaflet") {
						activeLegends[c.legends_field] = c.chart_instance.legend.legendItems.filter(t=> t.text !== "" && t.hidden === false).map(l=> l.text)
					}

				})
				
				params_chart.chart_type !== "leaflet" ? activeLegends[params_chart.legends_field] = params_chart.chart_instance.legend.legendItems.filter(t=> t.text !== "" && t.hidden === false).map(l=> l.text) : {}
			}
		...
		if (Object.values(filter_array_ref).length > 0 && params_chart.chart_type !== 'leaflet') {

			crossfilter(params_chart, params_chart, params_chart.filtered_by.axis, ind_filtering_type)
		}
		...
		function crossfilter(params_chart, params_chart_target, filter_array, ind_filtering_type) {
			...
			if (params_chart_target.list_labels_segment_single_selected.length !== 0 || params_chart_target.list_labels_segments_multiples_selected.length !== 0 
				...
				params_chart_target.chart_type !== "leaflet" ? hide_legends(params_chart_target, 'set_filter') : params_chart_target.instanciator.setup_legends(params_chart_target, 'update')
	...
	remove_filter(params_chart, params_charts_target, interaction_type) {
		...
		function crossfilter_rm(params_chart, params_chart_target, filter_array, ind_filtering_type) {
			...
			if (params_chart_target.list_labels_segment_single_selected.length !== 0 || params_chart_target.list_labels_segments_multiples_selected.length !== 0 
				...
				var data_type = "data"; var injection_type = "update"
				params_chart_target.instanciator.inject_metadata(chart_to_filter, params_chart_target, data_type, injection_type) // -> ok
				params_chart_target.chart_type === "leaflet" ? params_chart_target.instanciator.setup_legends(params_chart_target, 'update') : {}
				params_chart_target.prepare_data_type = ""
