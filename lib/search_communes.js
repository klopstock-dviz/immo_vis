...
async function chargerDataDetailleesCommunes() {
    ...
    communes_selected = []; data_annonces_details_ventes = []; dep_Loaded_data_details = ["init"]

    for (i=0; i < limit; i++) {
      var code_dep = list_villes_selected[i].code_dep;
      communes_selected.push(list_villes_selected[i].code_commune)

      if (code_dep !== undefined && dep_Loaded_data_details.indexOf(code_dep) === -1) {
            try{ 
              ...
              dep_Loaded_data_details.push(code_dep);
  ...
  data_annonces_details_ventes.map(r=> {r.taux_rendement_n7 = r.taux_rendement_n7 * 100})
  ...
  if (sharedParams.setup_graphics === true) {
    sharedParams1.transformations.filter.filter(f=> f.field === "INSEE_COM")[0].values = communes_selected; 
    sharedParams2.transformations.filter.filter(f=> f.field === "INSEE_COM")[0].values = communes_selected
    updateCharts1(sharedParams1); updateCharts2(sharedParams2)
  }
  //create
  else {
    setup_graphics1(communes_selected); setup_graphics2(communes_selected)
  ...
  function drop_elem_selected() {
    ...
    for (v in list_villes_selected) {
      ...
      if (list_villes_selected[parseInt(v)].commune === this.innerHTML) {        
        communes_selected = communes_selected.filter(c=> c !== list_villes_selected[parseInt(v)].code_commune)
        list_villes_selected.splice(v, 1);
        this.remove()
      }
    }

    if (communes_selected.length > 0) {
      sharedParams1.transformations.filter.filter(f=> f.field === "INSEE_COM")[0].values = communes_selected; 
      sharedParams2.transformations.filter.filter(f=> f.field === "INSEE_COM")[0].values = communes_selected
      updateCharts1(sharedParams1); updateCharts2(sharedParams2)
    }
  ...
  function supprimerVilles() {
    ...
    data_annonces_details_ventes = []; communes_selected = []
  }
