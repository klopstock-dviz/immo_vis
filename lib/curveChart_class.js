	...
	inject_metadata(curveChart, params_curveChart, data_type, injection_type) {
		...
		if (injection_type === "init") {
			...
			try {
				...
				console.log("curveChart ko:" + error.stack)
				curveChart.config.data.datasets[0].data = []
				curveChart.config.data.datasets[0].label = []
				curveChart.config.data.datasets[0].backgroundColor = []			
				return	
			}			
