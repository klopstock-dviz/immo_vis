class curveChart {
	...
	createChart(params_curveChart, data_to_transform) {
		
		var data_filtred = this.prepare_data_p1(params_curveChart, data_to_transform)

		this.prepare_data_p2(data_filtred, params_curveChart)
	...
	updateChart(params_curveChart) {
		var data_filtred = this.prepare_data_p1(params_curveChart)
	...
	prepare_data_p2(data_input, params_curveChart) {
		...
		//répeter l'opération autant de fois qu'il y a de sous-catégories (nb_sous_categories)
			...
			params_curveChart.data[0].labels.push(categories)
        	...
        	if (Object.keys(params_curveChart.backgroundColorArray_source).length === 0) {
        		...
        		if (sharedParams.used_color_schemes.hasOwnProperty(colored_axis) === true) {
	        		var backgroundColorArray_source = generateColors(nb_sous_categories, sharedParams.used_color_schemes[colored_axis], params_curveChart.colorsConfig.colorsOrder, colored_axis)
