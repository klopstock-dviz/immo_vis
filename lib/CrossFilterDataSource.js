            ...
            function(value, key) {
                if (key.indexOf("binned") > -1) {
                    ...
                    var result = value.map(v=> {
                        ...
                        var valueMax = parseFloat(v.substring(pos_sep+1, 20));
...
function prepare_engage_crossfilter(data_chart, params_chart, filterList, data_chuncks) {
    //stat exec time
    ...
    if (filter_binned_fields.length > 1) {              
        ...
        filter_binned_fields.map(binned_field=> { 
            ...
            var data_chunck = getFiltredData(data_chart, final_filterList, params_chart.id)
        ...
        var data_chuncks = getFiltredData(data_chart, filterList, params_chart.id);
...
function getFiltredData(data_chart, filterList, params_chart_id) {
