...
class Observe_Charts_state {
	...
	observe_chart_state(params_chart, params_charts_target) {
		...
		setInterval(function() {
			...
			try{
				...
				var legends_state = collect_active_legends(current_chart, _this)
				var active_legends = legends_state["active_legends"]; var hidden_legends = legends_state["hidden_legends"]

				var legends_hidden = detect_hidden_legends(current_chart); legends_hidden = deduplicate_array(legends_hidden)
				...
				// function collect_active_legends(current_chart) {
	...
	set_filter(params_chart, params_charts_target, interaction_type) {
		...
		for (var i = 0; i < params_charts_target.length; i++) {
			...
			collect_active_legends_setFilter(params_chart, params_chart_target, params_charts_target, filter_array, interaction_type)
	...
	remove_filter(params_chart, params_charts_target, interaction_type) {
		...
		for (var i = 0; i < params_charts_target.length; i++) {
			...
			//---------------------2. else if the target chart has hidden legends, restore it's original dataset with hidden legends
				...
				//else if the target chart has no active selection, allow its filtering
					...
					/*params_chart_target.list_keys_values_segments_multiples_selected.map(a=> filter_array[params_chart_target["legends_field"]] = a[params_chart_target["legends_field"]])
					filter_array_ref.hasOwnProperty(params_chart_target["legends_field"]) ? filter_array[params_chart_target["legends_field"]] = filter_array_ref[params_chart_target["legends_field"]] : {}*/
				}
		...
		for (var y = 0; y < params_charts_target.length; y++) {
			...
			var activeLegends={};
			params_charts_target.map(c=> activeLegends[c.legends_field] = c.chart_instance.legend.legendItems.filter(t=> t.text !== "" && t.hidden === false).map(l=> l.text))
			...
			//regenerate filter_array_ref

			//1.regenerate active selections
			var activeSelections={};
			params_charts_target.map(l=> _.mapValues(l.list_keys_values_segments_multiples_selected, function(v,k) {return activeSelections[Object.keys(v)] = Object.values(v)[0]}))

			//2.regenerate active legends
			var activeLegends={};
			params_charts_target.map(c=> activeLegends[c.legends_field] = c.chart_instance.legend.legendItems.filter(t=> t.text !== "" && t.hidden === false).map(l=> l.text))

			//3.delete obsolete active legends from the original filter_array_ref, the ones that contains "" strings
			var fd = _.cloneDeep(filter_array_ref)
			Object.keys(filter_array_ref).forEach(key => filter_array_ref[key].findIndex(i=> i === "") > -1 ? delete filter_array_ref[key] : {});

			//4.agregate the diff dicts
			//4.1.put activeSelections into activeLegends, where active selections take prior on active legends
			Object.assign(activeLegends, activeSelections)
			Object.assign(filter_array_ref, activeLegends)
			...
			for (var w = 0; w < Object.keys(keys_values_params_barChart).length; w++) {
		...
		if (Object.values(filter_array_ref).length > 0) {
			//if the chart has hidden legends, delete the corresponding values from the filter array to avoid self filtering
			if (Object.values(params_chart.hidden_legends).length > 0) {
				if (params_chart.hidden_legends[params_chart.legends_field].length > 0) {
					//delete filter_array_ref[params_chart.category_field]; delete filter_array_ref[params_chart.sub_category_field]; delete filter_array_ref[params_chart.legends_field]
					console.log('delete hidden_legends' + params_chart.legends_field + ': ' + filter_array_ref[params_chart.legends_field] + 'canceled')

				}
			}


			//case when the chart is 
			//call back all active selections on the target charts in order to include them in the filter array		
			//params_charts_target.map(l=> _.mapValues(l.list_keys_values_segments_multiples_selected, function(v,k) {return f[Object.keys(v)] = Object.values(v)[0]}))

			crossfilter_rm(params_chart, params_chart, params_chart.filtered_by.axis, ind_filtering_type)
		...
		function crossfilter_rm(params_chart, params_chart_target, filter_array, ind_filtering_type) {
			...
			//3.collect last version of active legends after the chart update
			/*if (Object.values(params_chart_target.hidden_legends).length > 0) {var eval_target_hidden_legends = Object.values(params_chart_target.hidden_legends)[0].length}
			if (eval_target_hidden_legends) {
				params_chart_target.active_legends[params_chart_target.legends_field] = params_chart_target.chart_instance.legend.legendItems.filter(t=> t.text !== "" && t.hidden === false).map(l=> l.text)
			}*/
...
function collect_active_legends(current_chart, _this) {
	//collect current legends
	var legends_array=[]; var hidden_legends=[]
	//remove active legends					
	//params_chart.active_legends = {}
