class Map {
	...
	prepare_data_p1(params_chart, sharedParams, data_to_transform) {
		...
		if (params_chart.transformations.filter !== undefined) {
			...
			data_chuncks = getFiltredData(data_chart, filterList, params_chart.id, sharedParams)
		}
	...
	init_chart(params_chart) {		
		...
		if (params_chart.title) {
			...
			//2.create outer div
			var container = document.createElement('div'); container.style.display = 'grid'
			//3.create the title
			var titleContainer = document.createElement('div'); titleContainer.style.display = 'grid'
			var title = document.createElement('span'); title.id = params_chart.htmlNode + '_title'; title.style = 'align-self: center; justify-self: center'; title.innerHTML = params_chart.title
