class grouped_barChart {
	...
	createChart(params_barChart, data_to_transform) {
		
		var data_filtred = this.prepare_data_p1(params_barChart, data_to_transform)

		this.prepare_data_p2(data_filtred, params_barChart)
	...
	updateChart(params_barChart) {
		var data_filtred = this.prepare_data_p1(params_barChart)
	...
	prepare_data_p2(data_input, params_barChart) {
		...
		//répeter l'opération autant de fois qu'il y a de sous-catégories (nb_sous_categories)
        	...
        	if (Object.keys(params_barChart.backgroundColorArray_source).length === 0) {
        		...
        		if (sharedParams.used_color_schemes.hasOwnProperty(colored_axis) === true) {
	        		var backgroundColorArray_source = generateColors(nb_sous_categories, sharedParams.used_color_schemes[colored_axis], params_barChart.colorsConfig.colorsOrder, colored_axis)
	        		...
	        		var backgroundColorArray_source = generateColors(nb_sous_categories, params_barChart.colorsConfig.scheme, params_barChart.colorsConfig.colorsOrder, colored_axis)
					sous_categories.map(axis => {
						params_barChart.backgroundColorArray_source[axis] = select_generated_color(backgroundColorArray_source, i); 
						i++
						} )
