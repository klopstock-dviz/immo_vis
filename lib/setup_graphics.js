...
function setup_graphics1(communes_selected) {
    ...
    data_stats_insee_com.map(r=> {r.taux_vacance_locative = (r.P17_LOGVAC / r.P17_LOG)*100})

    //build lib iris
    sharedParams1.data_main.forEach(r=> {
      var LIB_IRIS = ref_insee.filter(r2=> r2.CODE_IRIS === r.CODE_IRIS)[0].LIB_IRIS 
      r['LIB_IRIS'] = LIB_IRIS
    })
    ...
    params_map1.popup_fields = {6:{fieldName: "taux_rendement_n7" ,textBefore: "Taux de rendement", textAfter: " %"}, 0:{fieldName: "LIB_IRIS", slice:[0, 15] ,textBefore: "Quartier"}, 1: {fieldName: "prix_bien", textBefore: "Prix du bien", textAfter: " €"},
...
function setup_graphics2(communes_selected) {
    ...
    //build lib iris
    sharedParams2.data_main.forEach(r=> {
      var LIB_IRIS = ref_insee.filter(r2=> r2.CODE_IRIS === r.CODE_IRIS)[0].LIB_IRIS 
      r['LIB_IRIS'] = LIB_IRIS
    })
...
params_choroplethe_map1.htmlNode = 'map2';
...
params_choroplethe_map1.tooltip_fields = {0:{fieldName: "LIB_IRIS", slice:[0, 15] ,textBefore: "Quartier", selection: "first"},
                                         1:{fieldName: "taux_rendement_n7" ,textBefore: "Tx de rendement", agg_type: "mean", toPrecision: 4},
                                          ...
                                          4:{fieldName: "CODE_IRIS", slice:[0, 15] ,textBefore: "Code Quartier", selection: "first"}}
params_choroplethe_map1.title = "Quartiers selon leur taux de rendement"
//params_map1.bounds_adjustment = {adjustment: true, domain: ["p0.05", "p0.95"]} //-> bounds_adjustment param help to exclude the coordinates that are out of the given domain
//here you can declare a set of transformations that will be applied to this particular chart
...
instantiateur_groupedBar1.createChart(params_groupedBar1, sharedParams2)
  ...
  params_SliderDoubleRange.title_params = {label: 'Taux de rendement', position: "top"}
  params_SliderDoubleRange.data_params = {dataset: undefined, range_field: 'taux_rendement_n7', domain: ['auto', 'auto']}
  params_SliderDoubleRange.range_start = ['p0.05', 'p0.98']
  //params_SliderDoubleRange.style = {margin: '30px 10px 10px 10px'}
  params_SliderDoubleRange.hr = {display: true, style: 'width:100%;text-align:left;margin-left:0'}
  /*range_start params; 
  ...
  */
  ...
  //params_SliderDoubleRange.cells_format = {decimals: 2, thousand_separator: '.', suffix: '%'} //if not definde, the decimals is 4, thousand_separator '.', and no suffix
  params_SliderDoubleRange.tooltips = {decimals: 2, thousand_separator: '.', suffix: '%'}
  ...
  //slider
  params_SliderDoubleRange2 = new params_slider()
  params_SliderDoubleRange2.htmlNode = 'slider2'
  params_SliderDoubleRange2.title_params = {label: 'Taux de pauvreté'}
  params_SliderDoubleRange2.data_params = {dataset: data_stats_insee_com, range_field: 'TP6017', domain: ['auto', 'auto'], range_adjustment: 10,
                                          join_params: {join_field_source: "CODGEO", join_field_target: "INSEE_COM", dataset_target: sharedParams2.data_main, selection: undefined}}
                                          
  params_SliderDoubleRange2.range_start = ['p0.05', 'p0.98']
  params_SliderDoubleRange2.step = 0.1
  //params_SliderDoubleRange2.style = {margin: '40px 10px 10px 10px'}
  params_SliderDoubleRange2.hr = {display: true, style: 'width:100%;text-align:left;margin-left:0'}
  //params_SliderDoubleRange2.update_handles_position = true
  /*update_handles_position option makes the slider handles adjust during the crossfilter process, by default this option is set at false
  !! caution, the internal set method of nouislider to adjust the positions of the handles accept only integer values, iot works only for a discret range scale*/
  /*range_start params; 
  by default, the min & max is equal to the dataset extent
  you can pass a domain array, like ['p0.09', 'auto'], where p0.01 is the percentile 10 of the field
  you can also pass raw values in num format, like min: 15, max: 85
  */
  ...
  //params_SliderDoubleRange2.cells_format = {decimals: 2, thousand_separator: '.', suffix: '%'} //if not definde, the decimals is 4, thousand_separator '.', and no suffix
  params_SliderDoubleRange2.tooltips = {decimals: 2, thousand_separator: '.', suffix: '%'}

  var instantiateur_slider2 = new SliderDoubleRange(params_SliderDoubleRange2)

  instantiateur_slider2.createChart(params_SliderDoubleRange2, sharedParams2)



  //slider
  params_SliderDoubleRange3 = new params_slider()
  params_SliderDoubleRange3.htmlNode = 'slider3'
  params_SliderDoubleRange3.title_params = {label: 'Taux de vacance locative'}
  params_SliderDoubleRange3.data_params = {dataset: data_stats_insee_com, range_field: 'taux_vacance_locative', domain: ['auto', 'auto'], 
                                          join_params: {join_field_source: "CODGEO", join_field_target: "INSEE_COM", dataset_target: sharedParams2.data_main, selection: undefined}}
                                          //the source is the dataset specified for the slider, the target is the fact table
                                          //the selection parameter is used for the source, iot select/aggregate the values for the specified field, before pushing them in the target dataset
                                          
  params_SliderDoubleRange3.range_start = ['auto', 'auto']
  //params_SliderDoubleRange3.style = {margin: '40px 10px 10px 10px'}
  params_SliderDoubleRange3.hr = {display: true, style: 'width:100%;text-align:left;margin-left:0'}
  /*range_start params; 
  by default, the min & max is equal to the dataset extent
  you can pass a domain array, like ['p0.09', 'auto'], where p0.01 is the percentile 10 of the field
  you can also pass raw values in num format, like min: 15, max: 85
  */
  ...
  //params_SliderDoubleRange3.cells_format = {decimals: 2, thousand_separator: '.', suffix: '%'} //if not definde, the decimals is 4, thousand_separator '.', and no suffix
  params_SliderDoubleRange3.tooltips = {decimals: 2, thousand_separator: '.', suffix: '%'}
  var instantiateur_slider3 = new SliderDoubleRange(params_SliderDoubleRange3)

  instantiateur_slider3.createChart(params_SliderDoubleRange3, sharedParams2)
