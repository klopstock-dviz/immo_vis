class mixed_BarLineChart_singleScale {
	...
	createChart(params_mixed_barChart, sharedParams) {
		...
		//processus de création d'un nouveau dataset: 
        	...
        	if (Object.keys(params_mixed_barChart.backgroundColorArray_barChart_source).length === 0) {
        		...
        		if (sharedParams.used_color_schemes.hasOwnProperty(colored_axis) === true) {
	        		var backgroundColorArray_source = generateColors(nb_categories, sharedParams.used_color_schemes[colored_axis], params_mixed_barChart.colorsConfig.colorsOrder, colored_axis, sharedParams)
					categories.map(axis => {
	        		...
	        		var backgroundColorArray_source = generateColors(nb_categories, params_mixed_barChart.colorsConfig.scheme, params_mixed_barChart.colorsConfig.colorsOrder, colored_axis, sharedParams)
					categories.map(axis => {
						params_mixed_barChart.backgroundColorArray_barChart_source[axis] = select_generated_color(backgroundColorArray_source, i); 
						i++
						} )
