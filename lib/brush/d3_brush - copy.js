function brush_scatterPlot(params_chart, htmlNode) {
    ...
    //listener for activating the svg brush container on clic
        /*canvasChart.onmousedown = function(evt) {
            var canvasChart_id = chart_instance.canvas.id
            sharedParams.interaction_events[canvasChart_id].type_event.click = true
        }
        svgSceen.onmousedown = function(evt) {
            var canvasChart_id = chart_instance.canvas.id
            sharedParams.interaction_events[canvasChart_id].type_event.click = true
        }
        canvasChart.onmouseup = function(evt) {
            var canvasChart_id = chart_instance.canvas.id
            sharedParams.interaction_events[canvasChart_id].type_event.click = false
        }
        svgSceen.onmouseup = function(evt) {
            var canvasChart_id = chart_instance.canvas.id
            sharedParams.interaction_events[canvasChart_id].type_event.click = false
        }    */    
...
function restorColors(params_chart) {
    ...
    if (sharedParams.interaction_events[canvasChart_id].brushed === true) {
        params_chart.chart_instance.data.datasets.map(dset=> {
            ...
            if (label !== "") {
                var backgroundColorArray = [...params_chart.data[1].datasets.filter(pdset => pdset.label === dset.label)[0].backgroundColor]
