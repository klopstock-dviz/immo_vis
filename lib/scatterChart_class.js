		...
		//répeter l'opération autant de fois qu'il y a de sous-catégories (nb_sous_categories)
        	...
        	if (Object.keys(params_chart.backgroundColorArray_source).length === 0) {
        		...
        		if (sharedParams.used_color_schemes.hasOwnProperty(colored_axis) === true) {
	        		var backgroundColorArray_source = generateColors(nb_categories, sharedParams.used_color_schemes[colored_axis], params_chart.colorsConfig.colorsOrder, colored_axis, sharedParams)
					categories.map(axis => {
	        		...
	        		var backgroundColorArray_source = generateColors(nb_categories, params_chart.colorsConfig.scheme, params_chart.colorsConfig.colorsOrder, colored_axis, sharedParams)
					categories.map(axis => {
						params_chart.backgroundColorArray_source[axis] = select_generated_color(backgroundColorArray_source, i); 
						i++
						} )
		...
		if (injection_type === "init") {
			...
			for (var i = 0; i < l; i++) {
				try {
					...
					scatterChart.config.data.datasets.push({label: datasets[i].label, data: datasets[i].data, backgroundColor: datasets[i].backgroundColor})
				}				
			}
