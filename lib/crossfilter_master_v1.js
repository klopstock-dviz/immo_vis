...
function crossfilter_master(params_chart, params_chart_target, filter_array, ind_filtering_type) {
    ...
    //define dynamique filter function
		...
		/*			if (filter[params_chart.category_field] !== undefined) {
	    	...
	    	/*in the filtering process below, we filter the raw data source according to the nb of axis of the source chart 
	    	(chart on which the clic has been performed and that fires the whole actual process)
	    	Before the final filter function, we must check if the source chart is already filtred by another chart, in order
	    	to preserve this filter value*/

	    	//if the primary chart has one categorical axis, use this filter method
		    if (params_chart.nb_axis === 1) {
				var category_field_array = []; var category_field = "";

				//if the source chart is already filtred by another chart, preserve the filter by testing the two conditions above
				//1.case when the source chart is not filtred by another chart
				if (Object.keys(params_chart.filtered_by).length === 0) {
					category_field_array = deduplicate_dict(data_annonces_details_filtre, params_chart.category_field);
				}
				//2.case when the source chart is filtred by another chart
				else if (Object.keys(params_chart.filtered_by).length === 1){
					category_field = Object.keys(params_chart.filtered_by)[0]; category_field_array = Object.values(params_chart.filtered_by)[0];
				}


				//fill the filter dict with the fields to be used in the filtering function (the value of axis clicked in the primary chart, stored in params_chart)
				if (category_field === "") {category_field = params_chart.category_field};
			...
			//if the primary chart has two categorical axis, use this filter method
				...
				//if the source chart is already filtred by another chart, preserve the filter by testing the three conditions above
				//1.case when the source chart is not filtred by another chart
				if (Object.keys(params_chart.filtered_by).length === 0) { 
					category_field_array = deduplicate_dict(data_annonces_details_filtre, params_chart.category_field)
					sub_category_field_array = deduplicate_dict(data_annonces_details_filtre, params_chart.sub_category_field)
				}
				//2.case when the source chart is filtred by another chart with one field
				else if (Object.keys(params_chart.filtered_by).length === 1) {
					category_field = Object.keys(params_chart.filtered_by)[0]; category_field_array = Object.values(params_chart.filtered_by)[0]
				}
				//3.case when the source chart is filtred by another chart with two fields
				else if (Object.keys(params_chart.filtered_by).length === 2) {
					category_field = Object.keys(params_chart.filtered_by)[0]; category_field_array = Object.values(params_chart.filtered_by)[0];
					sub_category_field = Object.keys(params_chart.filtered_by)[1]; sub_category_field_array = Object.values(params_chart.filtered_by)[1]
				}				


				//fill the filter dict with the fields to be used in the filtering process (the values of axis clicked in the primary chart, stored in params_chart)
				if (category_field === "") {category_field = params_chart.category_field};
				if (sub_category_field === "") {sub_category_field = params_chart.sub_category_field};
				if (category_field_array.length > 0) {filter[category_field] = category_field_array}
