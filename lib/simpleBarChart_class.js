		...
		*/			
			...
			if (params_barChart.backgroundColorArray_source["category_field"] !== params_barChart.category_field) { // && Object.keys(params_barChart.backgroundColorArray_source).length === 0
        		...
        		if (sharedParams.used_color_schemes.hasOwnProperty(colored_axis) === true) {
	        		var backgroundColorArray_source = generateColors(nb_categories, sharedParams.used_color_schemes[colored_axis], params_barChart.colorsConfig.colorsOrder, colored_axis)
					categories.map(axis => {
	        		...
	        		var backgroundColorArray_source = generateColors(nb_categories, params_barChart.colorsConfig.scheme, params_barChart.colorsConfig.colorsOrder, colored_axis)
					categories.map(axis => {
						params_barChart.backgroundColorArray_source[axis] = select_generated_color(backgroundColorArray_source, i); 
						i++
						} )
