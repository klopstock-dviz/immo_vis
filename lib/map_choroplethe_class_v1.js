class Map_choroplethe {
	...
	updateChart(params_chart) {
		...
		inject_metadata(params_chart.map_instance, params_chart)
			...
			//in case when hue_field is categorical, handle the domain extent and domain scope params
				...
				if (params_chart.polygons_colors_setup === undefined) {
					if (sharedParams.used_color_schemes.hasOwnProperty(color_field) === true) {
						var generated_colors = generateColors(nb_sous_categories, sharedParams.used_color_schemes[color_field]) }
					else {
						var generated_colors = generateColors(categories.length, color);}
	...
	setup_legends(params_chart, mode) {
		...
		generateLegends(legends_params, params_chart)
