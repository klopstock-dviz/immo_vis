	...
	prepare_data(data_input, params_barChart) {
		...
		//répeter l'opération autant de fois qu'il y a de sous-catégories (nb_sous_categories)
			...
			if (params_barChart.prepare_data_type === "preserve backgroundColor") {
				...
				var active_slices_sub_category_fields = deduplicate_dict(params_barChart.active_slices, "sub_category_field");
				var active_slices_bckgColors = deduplicate_dict(params_barChart.active_slices, "backgroundColor");
				...
				for (var i = 0; i < nb_sous_categories; i++) {
					...
					if (pos_sub_category > - 1) {
						// 3.2.with this pos a, get the category field value from the array of category fields, built in step 1
						var active_slices_category_field_value = active_slices_category_fields[pos_sub_category];
				 		//3.3.get the bckg color value from the array of bckg colors, built in step 1
						var active_slice_bckgColor = active_slices_bckgColors[pos_sub_category];
						...
						//params_barChart.data[1].datasets[i].backgroundColor[pos_category_field] = active_slice_bckgColor
